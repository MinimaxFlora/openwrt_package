...
enabled=`uci get mihomo.config.enabled`
...
`
if [ -x "/bin/opkg" ]; then
	opkg list-installed "mihomo"
	opkg list-installed "luci-app-mihomo"
elif [ -x "/usr/bin/apk" ]; then
	apk list -I "mihomo"
	apk list -I "luci-app-mihomo"
fi
`
\`\`\`
## config
...
const config = uci.get_all("mihomo");
const result = {};
...
TPROXY: 
`
ip route list table "$(uci get mihomo.routing.tproxy_route_table)"
`

TUN: 
`
ip route list table "$(uci get mihomo.routing.tun_route_table)"
`
\`\`\`
## ip6 rule
\`\`\`
...
TPROXY: 
`
ip -6 route list table "$(uci get mihomo.routing.tproxy_route_table)"
`

TUN: 
`
ip -6 route list table "$(uci get mihomo.routing.tun_route_table)"
`
\`\`\`
## nftables
\`\`\`
